...
public final class Speed extends Module {
    private final ModeSetting mode = new ModeSetting("Mode", "Watchdog",
            "Watchdog", "WatchdogOld", "Strafe", "Matrix", "HurtTime", "Vanilla", "BHop", "Verus", "Viper", "Vulcan", "Zonecraft", "Heatseeker", "Mineland");
    ...
    public Speed() {
        ...
        watchdogMode.addParent(mode, modeSetting -> modeSetting.is("WatchdogOld"));
        verusMode.addParent(mode, modeSetting -> modeSetting.is("Verus"));
        viperMode.addParent(mode, modeSetting -> modeSetting.is("Viper"));
        groundSpeed.addParent(watchdogMode, modeSetting -> modeSetting.is("Ground") && mode.is("WatchdogOld"));
    ...
    public void onMoveEvent(MoveEvent e) {
        ...
        if (mode.is("Watchdog")) {
            if(MovementUtils.isMoving()) {
                if(mc.thePlayer.onGround) {
                    inAirTicks = 0;
                    e.setSpeed(MovementUtils.getSpeed());
                }
            }
        }
    ...
    public void onPlayerMoveUpdateEvent(PlayerMoveUpdateEvent e) {
        if (mode.is("WatchdogOld") && (watchdogMode.is("Hop") || watchdogMode.is("Dev") || watchdogMode.is("Low Hop")) && mc.thePlayer.fallDistance < 1 && !mc.thePlayer.isPotionActive(Potion.jump)) {
            if (MovementUtils.isMoving()) {
                switch (watchdogMode.getMode()) {
                    case "Low Hop":
                    case "Hop":
    ...
    public boolean shouldPreventJumping() {
        return Tenacity.INSTANCE.isEnabled(Speed.class) && MovementUtils.isMoving() && !(mode.is("WatchdogOld") && watchdogMode.is("Ground"));
    }
