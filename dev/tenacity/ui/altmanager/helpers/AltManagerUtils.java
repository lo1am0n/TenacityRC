...
public class AltManagerUtils implements Utils {
    ...
    public void microsoftLoginAsyncTOKEN(String token) {
        microsoftLoginAsyncTOKENV2(null, token);
    }
    ...
    public void microsoftLoginAsyncTOKENV2(Alt alt, String token) {
        if (alt == null) {
            alt = new Alt("?", "?");
        }
        Alt finalAlt = alt;
        Multithreading.runAsync(() -> {
            ...
            MicrosoftLogin.LoginData login = MicrosoftLogin.loginToken(token);
            future.complete(new Session(login.username, login.uuid, login.mcToken, "microsoft"));
