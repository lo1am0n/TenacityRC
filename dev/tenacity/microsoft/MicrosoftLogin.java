...
public class MicrosoftLogin {
    ...
    public static LoginData loginToken(String token) {

        // Check game ownership
        GameOwnershipResponse gameOwnershipRes = gson.fromJson(
                Browser.getBearerResponse("https://api.minecraftservices.com/entitlements/mcstore", token),
                GameOwnershipResponse.class);
        ...
        // Profile
        ProfileResponse profileRes = gson.fromJson(
                Browser.getBearerResponse("https://api.minecraftservices.com/minecraft/profile", token),
                ProfileResponse.class);

        if (profileRes == null) return new LoginData();

        return new LoginData(token, "N/A", profileRes.id, profileRes.name);
    }
